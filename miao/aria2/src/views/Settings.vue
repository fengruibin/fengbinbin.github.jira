  ...
  <div>
    <dl>
      <template v-for="(val, key) in settings">
        <dt :key="key + 'a'">
          {{ optionDesc[key] ? optionDesc[key] + "(" + key + ")" : key }}
        </dt>
        <dd :key="key + 'b'">
          <div v-if="val == 'true' || val == 'false'">
            <label
              >是<input type="radio" v-model="settings[key]" value="true" />
            </label>
            <label
              >否<input type="radio" v-model="settings[key]" value="false" />
            </label>
          </div>
    ...
    <button @click="changeGlobalOption">保存</button>
  </div>
</template>
